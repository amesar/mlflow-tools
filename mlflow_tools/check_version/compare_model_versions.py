...
def compare_versions(config, download_dir, skip_compare_run_models=False, skip_compare_reg_models=False, report_file=None):
    local_utils.dump(config, "Config")
    if skip_compare_run_models and skip_compare_reg_models:
        print(f"WARNING: Nothing to compare: skip_compare_run_models={skip_compare_run_models} skip_compare_reg_models={skip_compare_reg_models}")
        return

    cfg1 = config["tracking_server_1"]
    ...
    if not skip_compare_run_models:
    ...
    if not skip_compare_reg_models:
...
@opt_download_dir
@click.option("--skip-compare-run-models", 
    help="Do not compare run models", 
    type=bool, 
    is_flag=True
)
@click.option("--skip-compare-reg-models", 
    help="Do not compare registry models", 
    type=bool, 
    is_flag=True
)
@opt_report_file
def main(config_file, download_dir, skip_compare_run_models, skip_compare_reg_models, report_file):
    """ 
    Compare two model versions: check that the cached registry models are the same and the run models are the same.
    """
    print("Options:")
    ...
    compare_versions(cfg, download_dir, skip_compare_run_models, skip_compare_reg_models, report_file)
