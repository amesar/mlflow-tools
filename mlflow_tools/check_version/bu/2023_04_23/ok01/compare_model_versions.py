...
from mlflow_tools.tools.check import tools_utils
from mlflow_tools.tools.check import io_utils
from mlflow_tools.tools.check import new_utils


# _logger = utils.getLogger(__name__)

def compare_versions(cfg, download_dir, compare_run_models, compare_reg_models, 
        use_deprecated_download_artifacts=False, 
        verbose=False
    ):
    tools_utils.dump(cfg,"Config")
    ...
    vr1 = versions1[0]
    if verbose:
        tools_utils.dump(vr1,"Client1 Version")
    versions2 = client2.get_latest_versions(cfg2["model"],[cfg2["stage"]])
    vr2 = versions2[0]
    if verbose:
        tools_utils.dump(vr2,"Client2 Version")

    if compare_run_models:
        run_res = do_compare_run_models(client1, client2, vr1, vr2, cfg1, cfg2, download_dir, use_deprecated_download_artifacts)
        tools_utils.dump(run_res, "Run_Model_Comparison")
    if compare_reg_models:
        reg_res = do_compare_reg_models(client1, client2, vr1, vr2, cfg1, cfg2, download_dir)
        tools_utils.dump(reg_res, "Reg_Model_Comparison")
...
def do_compare_run_models(client1, client2, vr1, vr2, cfg1, cfg2, download_dir, use_deprecated_download_artifacts=False):
...
def download_artifact(client, download_uri, cfg, download_dir, model_type, idx):
    ...
    local_path = new_utils.download_artifacts(client, download_uri, dst_path, fix=True)
...
def compare_paths(path1, path2):
    if os.path.isdir(path1):
        ...
        compare = tools_utils.compare_dirs(path1, path2)
        equals = compare["equals"]
        print(">> compare_paths: EQUALS dir:",equals)
    else:
        equals = tools_utils.compare_files(path1, path2)
...
@click.option("--use-deprecated-download-artifacts", help="use_deprecated_download_artifacts", type=bool, is_flag=True)
@click.option("--verbose", help="Verbose", type=bool, is_flag=True)

def main(config_file, download_dir, compare_run_models, compare_reg_models, use_deprecated_download_artifacts, verbose):
    """ 
    ...
    compare_versions(cfg, download_dir, compare_run_models, compare_reg_models, use_deprecated_download_artifacts, verbose)
